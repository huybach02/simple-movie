        ...
        <div className='pb-10 page-container'>
            <div className="flex mb-10">
                <div className="flex-1">
                    <input type="text" className='w-full p-4 bg-slate-800 outline-none text-white rounded-tl-lg rounded-bl-lg' placeholder='Type here to search film...' onChange={handleInputChange} />
                </div>
            ...
            <div className="grid grid-cols-4 gap-10">
                {!loading && movies && movies.length > 0 && movies.map((item) => (
                    <MovieCard
                        key={item.id}
                        item={item}>
                    </MovieCard>
