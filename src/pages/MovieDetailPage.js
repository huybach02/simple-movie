        ...
        <>
            <div className="w-full h-[600px] relative">
                ...
                <div
                    ...
                    style={{
                        backgroundImage: `url(${API.imageOriginal(
                            data.backdrop_path
                        )})`,
                    }}
                ></div>
            ...
            <div className="w-full h-[400px] max-w-[1000px] mx-auto -mt-[200px] relative z-10 mb-10">
                <img
                    src={API.imageOriginal(data.poster_path)}
                    alt=""
            ...
            <div className="flex items-center justify-center gap-x-5 mb-10">
                {data.genres.length > 0 &&
                    data.genres.map((item) => (
                        <span
                            className="py-2 px-4 border-primary bg-primary rounded-lg"
                            key={item.id}
                        >
                            {item.name}
                        </span>
            ...
            <div className="grid grid-cols-4 gap-5">
                {data.cast.slice(0, 4).map((item) => (
                    <div key={item.id} className="cast-item">
                        <img
                            src={API.imageOriginal(item.profile_path)}
                            alt=""
                            className="w-full h-[350px] object-cover rounded-lg mb-3"
                        />
                        <h3 className="text-center text-lg mb-16">
                            {item.name}
                        </h3>
                    </div>
                ))}
    ...
    return (
        <div className="">
            <h2 className="text-center text-3xl font-bold mb-10">
                Trailers and videos about this movies
            </h2>
            {data.results.slice(0, 3).map((item) => (
                <div
                    key={item.id}
                    className="lg:w-[1000px] aspect-video mx-auto mb-10"
                >
                    <h3 className="mb-5 text-xl font-medium bg-primary inline-block px-4 py-2 rounded-lg">
                        {item.name}
                    </h3>
                    <iframe
